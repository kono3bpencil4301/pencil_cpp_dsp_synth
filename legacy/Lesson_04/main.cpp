...
int main()
{
    try
    {
        ...
        voice.setAttackCurveType(CurveType::EaseIn);
        voice.setDecayCurveType(CurveType::EaseIn);
        voice.setReleaseCurveType(CurveType::EaseOut);
        // 归一化曲线强度 [0, 1]：0 = 线性，0.5 = 二次，0.75 = 四次
        voice.setCurveStrength(0.75f, 0.55f, 0.65f);
        voice.setADSRAll(1.0f, 2.0f, 0.1f, 4.0f);

        // ---------- 逐采样点生成音频 ----------
        auto buffer = voice.generate(60, 0.8f, 4.0f);
